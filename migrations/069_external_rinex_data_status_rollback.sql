-- Rollback for migration 069: restore the 068 view (external RINEX-only
-- stations read N/A on status_24h / rinex_24h_status again).
--
-- Re-applies the 068 view definition verbatim (CREATE OR REPLACE, so grants and
-- dependent views survive) and removes the 069 marker row.
--
-- WARNING: rolling back restores the defect 069 fixes -- external stations
-- (MYVA, KRAC, THRC, TORK) read -2 (N/A) on the data-flow columns again. Roll
-- back only if 069 itself misbehaves, not to "quiet" the board.
--
...
DELETE FROM schema_migrations WHERE migration_name = '069_external_rinex_data_status';
