-- Migration 069: external (RINEX-only) stations get a real data-flow status
--
-- Problem: external stations (``health_check = passive``, data pulled from a
-- third-party server via ``receivers.external_fetch``) have no ``receiver_type``
-- and no ``15s_24hr`` RAW files -- they receive RINEX directly
-- (``external_data_type = rinex``), tracked in file_tracking as
-- ``session_type = '15s_24hr_rinex'``.
--
-- The ``status_24h`` / ``rinex_24h_status`` CASE expressions assume the normal
-- "raw -> RINEX" pipeline and short-circuit to -2 (N/A) for any station with no
-- receiver_type and no raw files:
--
--     WHEN fh.receiver_type IS NULL
--          AND NOT COALESCE(l.session_15s_24hr, false)
--          AND r24.file_date IS NULL THEN -2
--
-- So MYVA (LMI), KRAC/THRC/TORK (Háskóli Íslands) always read N/A on the map's
-- data circle and the detail dashboard, no matter how current their RINEX is.
-- The map additionally hard-codes ``health_check = 'passive' -> -1`` for the
-- data circle (fixed separately in the dashboard JSON), leaving no way to see
-- that an external station has stopped delivering.
--
-- Fix: give RINEX-only stations (raw absent, RINEX present) a real status from
-- the RINEX file date, evaluated BEFORE the no-receiver/N/A branch. Daily
-- external sources publish once per day, so:
--   status_24h:        green >= yesterday, yellow 2-3 days, red 4+ days
--   rinex_24h_status:  green >= yesterday, yellow 2 days,    red 3+ days
--
-- A normal station always has raw files, so ``r24.file_date IS NULL`` is false
-- for it and this branch never fires -- the existing raw->RINEX ladder is
-- untouched. Only the two 24h CASE expressions change; column names, order and
-- types are identical, so CREATE OR REPLACE VIEW is used (preserves object-level
...
base AS (
    SELECT fh.sid,
        ...
        -- status_24h (FIXED in 069 -- external RINEX-only stations)
        CASE
            WHEN fh.station_status IS NOT NULL THEN -2
            -- External / RINEX-only station (no raw pipeline): evaluate data
            -- flow on RINEX recency, not the raw->RINEX ladder. Daily sources:
            --   green  >= yesterday, yellow 2-3 days, red 4+ days.
            WHEN r24.file_date IS NULL AND x24.file_date IS NOT NULL THEN
                CASE
                    WHEN x24.file_date >= CURRENT_DATE - 1 THEN 0
                    WHEN x24.file_date >= CURRENT_DATE - 3 THEN 1
                    ELSE 2
                END
            WHEN fh.receiver_type IS NULL
                 AND NOT COALESCE(l.session_15s_24hr, false)
        ...
        CASE
            ...
            ELSE 2
        END AS status_1hz,
        -- rinex_24h_status (FIXED in 069 -- external RINEX-only stations)
        CASE
            WHEN fh.station_status IS NOT NULL THEN -2
            -- External / RINEX-only station: green >= yesterday, yellow 2 days,
            -- red 3+ days. No grace period (there is no raw->RINEX conversion
            -- to wait on -- the pulled file IS the RINEX).
            WHEN r24.file_date IS NULL AND x24.file_date IS NOT NULL THEN
                CASE
                    WHEN x24.file_date >= CURRENT_DATE - 1 THEN 0
                    WHEN x24.file_date >= CURRENT_DATE - 2 THEN 1
                    ELSE 2
                END
            WHEN fh.receiver_type IS NULL
                 AND NOT COALESCE(l.session_15s_24hr, false)
...
VALUES ('069_external_rinex_data_status')
